...
if ($Action -eq "stop") {
    Info "stopping spin-python-app..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-python-app?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-python-app.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-python-app/evaluate?ForceReschedule=true" | Out-Null
