...
$ImageTag = "spin-payment7-service:latest"
$NomadAddr = "http://localhost:4646"

function Info($msg) { Write-Host "[INFO] $msg" -ForegroundColor Green }

if ($Action -eq "stop") {
    Info "stopping spin-payment7-service..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-payment7-service?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-payment7-service.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-payment7-service/evaluate?ForceReschedule=true" | Out-Null
...
Write-Host ""
Info "http://localhost:3514/v1.0/invoke/spin-payment7-service/method/health"