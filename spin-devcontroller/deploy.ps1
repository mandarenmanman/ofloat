...
$ImageTag = "spin-devcontroller:latest"
$NomadAddr = "http://localhost:4646"

function Info($msg) { Write-Host "[INFO] $msg" -ForegroundColor Green }

if ($Action -eq "stop") {
    Info "stopping spin-devcontroller..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-devcontroller?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-devcontroller.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-devcontroller/evaluate?ForceReschedule=true" | Out-Null
...
Write-Host ""
Info "http://localhost:3507/v1.0/invoke/spin-devcontroller/method/health"
