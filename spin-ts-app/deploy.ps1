...
$ImageTag = "spin-ts-app:latest"
$NomadAddr = "http://localhost:4646"

function Info($msg) { Write-Host "[INFO] $msg" -ForegroundColor Green }

if ($Action -eq "stop") {
    Info "stopping spin-ts-app..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-ts-app?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-ts-app.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-ts-app/evaluate?ForceReschedule=true" | Out-Null
...
Write-Host ""
Info "http://localhost:3505/v1.0/invoke/spin-ts-app/method/health"
