...
$ImageTag = "spin-payment5-service:latest"
$NomadAddr = "http://localhost:4646"

function Info($msg) { Write-Host "[INFO] $msg" -ForegroundColor Green }

if ($Action -eq "stop") {
    Info "stopping spin-payment5-service..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-payment5-service?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-payment5-service.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-payment5-service/evaluate?ForceReschedule=true" | Out-Null
...
Write-Host ""
Info "http://localhost:3512/v1.0/invoke/spin-payment5-service/method/health"
