...
$ImageTag = "spin-regist-service:latest"
$NomadAddr = "http://localhost:4646"

function Info($msg) { Write-Host "[INFO] $msg" -ForegroundColor Green }

if ($Action -eq "stop") {
    Info "stopping spin-regist-service..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-regist-service?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-regist-service.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-regist-service/evaluate?ForceReschedule=true" | Out-Null
...
Write-Host ""
Info "wsl curl -s http://localhost:3517/v1.0/invoke/spin-regist-service/method/health"
