...
$ImageTag = "spin-dev-service:latest"
$NomadAddr = "http://localhost:4646"

function Info($msg) { Write-Host "[INFO] $msg" -ForegroundColor Green }

if ($Action -eq "stop") {
    Info "stopping spin-dev-service..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-dev-service?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-dev-service.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-dev-service/evaluate?ForceReschedule=true" | Out-Null
...
Write-Host ""
Info "wsl curl -s http://localhost:3516/v1.0/invoke/spin-dev-service/method/health"
