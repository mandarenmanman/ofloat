...
$ImageTag = "spin-user-service:latest"
$NomadAddr = "http://localhost:4646"

function Info($msg) { Write-Host "[INFO] $msg" -ForegroundColor Green }

if ($Action -eq "stop") {
    Info "stopping spin-user-service..."
    curl.exe -s -X DELETE "$NomadAddr/v1/job/spin-user-service?purge=true" | Out-Null
    Info "stopped"
    exit 0
}
...
$hcl = [System.IO.File]::ReadAllText("$ScriptDir\spin-user-service.nomad.hcl")
...
if ($result.EvalID) {
    ...
    curl.exe -s -X POST "$NomadAddr/v1/job/spin-user-service/evaluate?ForceReschedule=true" | Out-Null
...
Write-Host ""
Info "http://localhost:3507/v1.0/invoke/spin-user-service/method/health"
