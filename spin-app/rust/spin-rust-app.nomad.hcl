...
EOF
    ...
    task "dapr-sidecar" {
      ...
      config {
        ...
        entrypoint = ["/bin/sh", "-c"]
        args       = [
          "/usr/local/bin/daprd -app-id spin-app -app-port 80 -dapr-http-port 3500 -dapr-grpc-port 50001 -metrics-port 9091 -placement-host-address ${PLACEMENT_ADDR} -resources-path /local/components -config /local/config/config.yaml"
        ]
      }
